...
func TestV3NewPoint(t *T.T) {
	t.Run("valid-fields", func(t *T.T) {
		...
		pt := NewPointV3("abc", [][]any{
			...
			[]any{"tag1", "val-1", true},
		}, WithTime(time.Unix(0, 123)), WithPointPool(pp))

		t.Logf("pt: %s", pt.Pretty())

		assert.Equal(t, int64(123), pt.Get("f1").(int64))
		assert.Equal(t, 1.23, pt.Get("f2").(float64))
		assert.Equal(t, "str", pt.Get("f3").(string))
		assert.Equal(t, false, pt.Get("f4").(bool))
		assert.Equal(t, []byte("some-binary-data"), pt.Get("f5").([]byte))
		assert.Equal(t, "val-1", pt.GetTag("tag1"))
		assert.Equal(t, int64(123), pt.Time().UnixNano())
		assert.Len(t, pt.pt.Warns, 0)
	})

	t.Run("invalid-fields", func(t *T.T) {
		pp := NewPointPoolLevel3()
		...
		pt := NewPointV3("", [][]any{
			...
			[]any{"f1"},
			[]any{"f2", nil},
			[]any{"not-tag", "val-1", false},
		}, WithTime(time.Unix(0, 123)), WithPointPool(pp))

		t.Logf("pt: %s", pt.Pretty())

		assert.Len(t, pt.pt.Fields, 1)
		assert.Equal(t, "", pt.GetTag("not-tag"))
		assert.Len(t, pt.pt.Warns, 4)
	})
}

func BenchmarkNewPointV3(b *T.B) {
	pp := NewPointPoolLevel3()
	...
	b.Run("v3", func(b *T.B) {
		for i := 0; i < b.N; i++ {
			pt := NewPointV3("abc", [][]any{
				...
				[]any{"f4", false},
				[]any{"f5", []byte("some-binary-data")},
			}, WithPointPool(pp), WithPrecheck(false))
			pp.Put(pt)
		}
	})

	b.Run("not-v3", func(b *T.B) {
		for i := 0; i < b.N; i++ {
			pt := pp.Get()

			pt.SetName("m1")

			pt.AddKVs(pp.GetKV("f1", 123),
				pp.GetKV("f2", 1.23),
				pp.GetKV("f3", "str"),
				pp.GetKV("f4", false),
				pp.GetKV("f5", []byte("some-binary-data")),
			)
		...
		for i := 0; i < b.N; i++ {
			pt := NewPointV2("abc",
				KVs{
					...
					NewKV("f3", "str"),
					NewKV("f4", false),
					NewKV("f5", []byte("some-binary-data")),
				})
			_ = pt
		}
	})
