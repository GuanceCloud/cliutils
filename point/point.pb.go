	...
	if m.X != nil {
		{
			...
			i -= size
			if i < 0 {
				return -1, ErrPBMarshalOutOfRange
			}
	...
	if m.Val != nil {
		{
			...
			if i < 0 {
				return -1,
					fmt.Errorf("%w: kv: %+#v, size: %d, buf size: %d",
						ErrPBMarshalOutOfRange, m, size, len(dAtA))
			}
...
func (m *PBPoints) MarshalTo(dAtA []byte) (int, error) {
	...
	n, err := m.MarshalToSizedBuffer(dAtA[:size])
	if n != size {
		return 0, fmt.Errorf("%w: expect %d, got %d", ErrPBSizeNotEqual, size, n)
	}
	return n, err
}
...
var (
	...
	ErrPBMarshalOutOfRange       = fmt.Errorf("marshal out of range")
	ErrPBSizeNotEqual            = fmt.Errorf("encode size not equal to data size")
)
